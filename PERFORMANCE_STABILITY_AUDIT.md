...
| **Partial or open** | SB-09 (long-task instrumentation + lifecycle gate landed; `perf-budgets.json` baselines and the nightly compare lane remain unwired), SB-13 (budget breach stands; no media diet yet), SB-15 (lunara/ocean toggle heap flat; **stellar-drift WebGL-lane characterized 2026-07-25** — A/B fixed one real leak (`EffectComposer.dispose()` never disposes added passes; new `disposeComposerPasses` helper + tests; removes ~0.9 MB/12 toggles), and retainer + idle-vs-toggle analysis resolved the rest: the earlier "big" residual was mostly a DevTools-console measurement artifact + gameplay-time `demoRecorder` accumulation; genuine theme-switch leak on real HW is only **~0.16 MB/toggle**, mechanistically the three `Pass.js` shared-`_geometry` `onGeometryDispose`-listener accumulation (classic-renderer analog of the documented BloomNode upstream gap — left as an upstream item). The 35→49.5 MB figure was SwiftShader/WebGPU, not reproduced on real HW; WebGPU lane not re-measured here (iGPU TDR risk). ocean & swedish-forest `onDeviceLost` now halt their loops — see remediation log addenda) |
