...
| **Partial or open** | SB-09 (**closed 2026-07-26**: long-task instrumentation + lifecycle gate landed earlier; owner captured the first committed real-GPU baselines (RTX 5080, `reports/odyssey-perf/`); now wired — `perf:budgets:gate` in `pages.yml` (compare self-test + budgets structural lint + committed idle baselines re-checked vs declared budgets, honest SKIP when pending) and `perf:odyssey:nightly` for the real-hardware compare lane with a documented scheduler command; remaining owner step = actually registering the nightly task on the capture machine), SB-13 (budget breach stands; no media diet yet), SB-15 (lunara/ocean toggle heap flat; **stellar-drift dominant leak found & fixed 2026-07-26** — the audit's 35→49.5 MB leak was the **WebGPU lane**, caused by a redundant `device.lost.then((info) => this.handleDeviceLoss(info))` in `setupRendererResilience()` pinning the whole theme+scene on the never-settling `device.lost` promise; removed (recovery still covered by `renderer.onDeviceLost`). Software-WebGPU A/B: **+29.3 → +2.1 MB / 10 toggles (~93% eliminated)**, regression-tested. The **WebGL** lane's smaller `EffectComposer` pass-dispose leak was fixed 2026-07-25 (`disposeComposerPasses`); its residual (~0.16 MB/toggle) is the three `Pass.js` shared-`_geometry` listener drip (upstream). Both lanes' app-side leaks now fixed + measured; remaining bits are three-internal (filed upstream) or the separate gameplay-time `demoRecorder` accumulation. ocean & swedish-forest `onDeviceLost` now halt their loops — see remediation log addenda) |
...
> **RESOLUTION (2026-07-17, this branch):** deeper analysis reclassified **all 28 findings as false positives of stale heuristics**, not theme bugs. (a) "stop() but no cleanup() override" predated the current dispose chain — `disposeThemeInstance() → cleanup() → releaseInactiveResources() → this.stop()` (`base-theme.js:301, 492`) *does* run every stop() override at terminal disposal, and all 22 flagged themes chain to `super.stop()` (grep-verified), so the base safety nets run too. (b) All 4 "raw resize listener" findings are paired with removals the regex didn't recognize (`handleResize`, `boundResize`, or the `eventUnsubscribers.push(() => removeEventListener(...))` closure pattern). The runtime evidence agrees: the flagged set did not correlate with the S8-measured leaks (lunara was flagged but its dispose was verified correct — its leak was three-internal, SB-15; ocean leaked and was not flagged). Remediation shipped instead: the checker now enforces the *real* contract (`stop()`/`cleanup()` must chain to super; inline `bind(this)` listeners; resize listeners need a removal path), its heuristics are unit-tested (`tests/unit/theme-lifecycle-audit.test.js`, 10 cases), it passes on the current tree, and it now runs in CI (`pages.yml`). The finding's *enforcement-gap* half is closed; the "name-convention safety nets" observation stands as background for future theme work.
