...
- **P0.2** — the startup ident, the Single Player **T-piece menu icon**, the favicon **mosaic artwork**, and external/store branding are **unchanged**. *(Per the owner's decision, the seven-piece intro animation and the main menu are being retained for now.)* The **legacy-canvas grid renderer** previously listed here has been **removed** as verified dead code (see the merged-items list above and AV-2), so that sub-item is closed.
