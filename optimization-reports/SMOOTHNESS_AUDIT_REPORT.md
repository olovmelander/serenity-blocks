I'll write the engineering report directly from the confirmed findings and best-practices data provided. No further investigation is needed since the audit is already adversarially verified.

# Serenity Blocks â€” Smoothness & Feel Engineering Report

## 1. Executive Summary

The 5-8 highest-leverage changes, one line each:

1. **Add a `.catch()` to the physics promise chain** (`game.js:722-731`) â€” a single transient throw during line-clear currently freezes the entire game forever; this is the worst player-facing failure and an almost-trivial fix.
2. **Add lock delay (~500ms) with move/step reset** (`game.js:531-596`) â€” pieces lock the instant they touch the stack; this is the single biggest game-feel deficiency, killing tucks, slides, spins, and finesse.
3. **Drive gamepad DAS from the single-player loop** (`game.js:778-783` / `gamepad-controller.js`) â€” holding a direction on a controller moves the piece exactly once; the flagship Steam mode is nearly unplayable on a gamepad.
4. **Fix Summer theme leaking its whole WebGL context + bloom render targets** (`summer-theme.js:1407-1411`) â€” repeated load/evict drives toward "Too many active WebGL contexts" and a hard crash; the worst GPU leak.
5. **Add `forceContextLoss()` after every theme `renderer.dispose()`** (`base-theme.js:329-338` + 37 themes) â€” disposed contexts linger until GC; cycling themes exhausts the context pool and cascades into breakage.
6. **Implement real SRS rotation + wall kicks + hold piece** (`game.js:480-520`, `SinglePlayerMode.js:588-600`) â€” rotation isn't SRS (no floor/I kicks, no T-spin) and hold is a no-op; two core mechanics modern players expect are missing.
7. **Stop force-overwriting the peer's own board every host snapshot** (`ffa-p2p-game-state.js:1009-1021`) â€” the local piece rubber-bands backward on every 30Hz snapshot for all non-host players; the most damaging online smoothness defect.
8. **Delete the two `console.log` calls in the collision hot path** (`game.js:72-76`) â€” unconditional logging during cascades causes avoidable micro-stutter at the most visually active moments.
...
|---|---|---|---|---|
| Physics promise has no `.catch` â†’ permanent freeze on any throw | `game.js:722-731` | High | Small | Hard freeze of the whole game on a single transient error; only escape is app restart |
...
| No lock delay / move reset â†’ instant lock on contact | `game.js:531-596` | High | Medium | No tucks/slides/spins; placement feels twitchy and punishing |
...
| Summer theme leaks GL context + bloom RTs on evict | `summer-theme.js:327,1102-1132,1384-1411` | Critical | Small | Progressive GPU growth â†’ "Too many WebGL contexts" â†’ crash |
| No `forceContextLoss()` after theme renderer dispose (37 themes) | `base-theme.js:329-338` | High | Trivial | Long-session slowdown â†’ context loss cascade breaking all rendering |
...
**No lock delay or move/step reset** â€” `game.js:531-596, 642-732`
...
*Impact:* At high levels (drop interval floors near 10ms) a piece cements the moment it grazes the stack â€” no tucks, slides, or spins possible; feels punishing/twitchy.
*Fix:* Add `lockTimer`, `lockResetCount`, `isGrounded` to GameState. In `processAutoDrop`, when grounded, accumulate `lockTimer += delta` (use the authoritative delta, **not** `performance.now()`, to preserve demo/replay determinism) and only call `lockPiece` when `lockTimer >= LOCK_DELAY` or `lockResetCount >= 15`. In `move()`/`rotate()`, reset the timer on a successful action while grounded, capped at 15 resets. Un-ground when a slide reopens space below. Reconcile with the Quadra time-based lock bonus (`game.js:648-656`) â€” consider measuring it from first-ground-contact rather than spawn. Expose `LOCK_DELAY` / reset cap in `constants.js`.

**Rotation is not SRS; no I/floor kicks, no T-spin** â€” `game.js:480-520`
...
*Fix:* Add a `rotation` field (0/1/2/3) to pieces. Implement the standard JLSTZ and separate I-piece SRS kick tables (see Â§4 for exact offsets), each entry an `(dx, dy)` **pair** â€” test `canPlacePiece(x+dx, y-dy)` (invert `dy` since this engine's +y is down). Iterate the 5 offsets in table order, apply the first that passes. O piece never kicks. Optionally add T-spin 3-corner detection + a `lastActionWasRotation` flag for scoring. **Determinism caveat:** `rotate()` runs in `DemoPlayer` and P2P â€” deploy atomically; existing replays may invalidate.

**Hold piece is a no-op** â€” `SinglePlayerMode.js:588-600`
...
**Soft drop hardcoded to 50ms** â€” `controls.js:81-83, 92-94, 123-130`
...
*Impact:* After clears, a buffered tap applies a frame late (teleport) and extra taps drop. (Held DAS is *not* lost â€” it auto-resumes.)
...
**Two console.logs in the collision hot path** â€” `game.js:72-76`
...
*Impact:* Avoidable per-action string interpolation + synchronous console I/O â†’ micro-stutter during cascades, badly so with DevTools open in Electron.
...
**Line-clear blocks input for the full animation** â€” `physics.js:600-883`
...
*Fix:* (1) Small FIFO input buffer drained on spawn (above). (2) Bigger win: decouple the cosmetic fade from the input/spawn gate â€” let the next piece spawn after a short fixed ARE (~60-100ms) while the fade renders on a separate layer; keep `isProcessingPhysics` guarding only board-mutating gravity. Cheap interim: shorten the fade (30/20/20) to cut dead time ~40%. *Best-practice:* modern competitive (TETR.IO) uses line-clear delay = 0; overlap clear animation with spawn/ARE so it never stalls input.

**Hybrid interpolation alpha is dead code** â€” `frame-rate-controller.js:342-371`
`_renderTick` computes an `alpha` that the single-player `renderUpdate(time, alpha)` ignores; the board draws at integer `piece.y`. *Refuted premise:* the board is **not** re-rasterized at high rate â€” it renders via Phaser's own ~60fps loop independent of the hybrid split, and a dual-layer dirty flag already exists. So there is **no board frame-time win** from removing the split.
...
**Per-cell closure/spread allocation in piece draw** â€” `base-board-scene.js:885-928`
...
*Impact:* ~12-30 short-lived closures + 1 object per frame â†’ incremental GC hitches over long sessions (small magnitude â€” tetromino shapes are tiny).
...
**FPS `recordFrame()` double-counted when overlay shown** â€” `main.js:1912-1923`
...
*Fix:* Have `startFPSMonitor` call read-only `getStats()` instead of `recordFrame()` â€” or simply remove the standalone `startFPSMonitor` RAF and the dead hybrid loop entirely.

**Monitor refresh detected once from a noisy 20-frame startup sample** â€” `frame-rate-controller.js:46-86`
...
*Fix:* On Electron (primary platform), set `monitorRefreshRate` from `screen.getPrimaryDisplay().displayFrequency` â€” expose `display.displayFrequency` in `electron/main.js:77`'s get-displays handler (currently omitted). For the web fallback, defer detection to first idle, discard first ~5 frames, sample ~60, use the **median** inter-frame delta, and re-sample if implausibly low.

### Three.js Memory / Theme Lifecycle

**Summer theme leaks GL context + composer RT + bloom (worst offender)** â€” `summer-theme.js:327, 1102-1132, 1384-1411`
Summer creates its own `WebGLRenderer` (a full GL context), an `EffectComposer` over a large `WebGLRenderTarget` (samples:8, HalfFloat), and an `UnrealBloomPass` (~5 RTs + materials). Its `cleanup()` is just `super.cleanup()`. It's not `resourceProfile:'heavy-gpu'`, so `releaseManagedGpuResources()` never runs, and base cleanup only handles `this.scene`/`this.postComposer` â€” Summer stores its composer in `this.composer`. **Zero** `dispose()` on renderer/composer/bloom.
*Impact:* Each loadâ†’evict (LRU `maxCachedThemes=2`) leaks one GL context + bloom/composer RTs permanently â†’ "Too many active WebGL contexts" crash + rising VRAM/stutter.
...
**37 themes create their own renderer but ZERO call `forceContextLoss()`** â€” `base-theme.js:329-338`
`grep forceContextLoss` across all themes = 0 matches; base disposal calls only `renderer.dispose()`. In Three.js, `dispose()` frees programs/state but the browser keeps the `WebGLRenderingContext` alive until GC or `forceContextLoss()`. Themes *are* disposed on switch (LRU eviction), so this is disposed-but-not-released contexts lingering until GC â€” GC-timing-dependent, not guaranteed every session.
...
*Fix:* In `releaseManagedGpuResources()`, after `renderer.dispose()` add a guarded `renderer.forceContextLoss()` (try/catch), then remove the canvas and null. Mirror in per-theme overrides that dispose their own renderer (`aurora-theme.js:762`, `bioluminescence-theme.js:1993`, etc.). Best long-term: a single `base-theme.disposeRenderer(renderer)` helper routing all teardown through dispose+forceContextLoss+DOM-removal+null. *Best-practice:* keep **one** renderer for the app's lifetime â€” never new-up a renderer per theme; pair disposal discipline with a `webglcontextrestored` handler that re-marks textures `needsUpdate=true`.

**Composer + bloom RT leaks in 6 themes** â€” `black-hole-theme.js:2986-3018, 4104-4183` (+ blood-moon, fall, solar-eclipse, luminous-tides, summer)
`EffectComposer.dispose()` (verified in bundled Three) disposes only its 2 RTs + copyPass â€” it **never** iterates `this.passes`. `UnrealBloomPass.dispose()` frees 11 HalfFloat RTs + ~9 materials. None of the 6 themes call `bloomPass.dispose()`. Composer RTs additionally leak where the theme pre-nulls `this.composer` before base disposal (black-hole `4144`, blood-moon `1141`, solar-eclipse `2170`).
*Impact:* Bloom mip-chain RTs + materials leak in all 6; composer RTs leak in 3 â€” several MB VRAM per cycle, compounding context pressure.
...
**Sakura fox GLTF geometry/material/textures never disposed** â€” `sakura-twilight-theme.js:3216-3246`
`stop()` detaches foxes via `scene.remove(fox.model)` without disposing their GLTF geometry/material/textures, and omits `super.stop()`. Because Sakura is heavy-gpu, the base `disposeThreeJSGroup(this.scene)` runs *after* `stop()` has already detached the foxes â€” so the base traversal can't reach them. Each activation loads a fresh `Fox.glb`.
*Impact:* Repeated Sakura activation accumulates orphaned fox GLTF GPU resources â€” a slow leak (worsened by theme auto-cycling).
...
*(All three below are latent â€” the relevant teardown/`destroy`/`cleanup` is never invoked today, and the app is a process-lifetime singleton. Fix opportunistically; they future-proof against any soft-reload/re-init.)*

**controls.js global listeners can never be removed** â€” `controls.js:258-468, 490-522`
...
*Fix:* Store bound named handlers on the instance, register those, and add a real `cleanup()` that removes them + `clearTimers()`. Cheap perf nit: cache modal `getElementById` lookups and precompute a reverse keyâ†’action map so keydown avoids `Object.keys().find` per event.

**App `cleanup()` never drains `cleanupHandlers`** â€” `main.js:4992-5018`
...
**GamepadController.destroy() removeEventListener no-ops** â€” `gamepad-controller.js:152-160, 2014-2019`
...
**Peer board overwritten by every host snapshot â€” reconciliation is dead** â€” `ffa-p2p-game-state.js:1009-1021, 1671-1718`
`syncFromHost()` â†’ `_applySnapshotState(state, {forceLocal:true})` overwrites the local player's grid/currentPiece/lockedPieces with 30Hz authoritative state, then calls `_reconcileLocalPlayer()` â€” which replays `inputHistory`, but **nothing ever pushes to `inputHistory`** (`sendInput` only writes `pendingInputs`). So the peer predicts, then ~33ms later the host snapshot teleports the piece back with zero replay.
*Impact:* Every non-host player's own piece rubber-bands backward 1-2 cells on every snapshot â€” the most damaging online defect.
...
**Binary delta snapshots sent unreliable but decoded as reliable** â€” `steam-networking.js:358-413, 486-521, 929-1004`
`GAME_STATE_FULL` is encoded as a binary delta vs `lastBroadcastSnapshot` and sent `unreliable_no_delay`, but the decoder assumes the previous packet was the baseline ("Since we use reliable deliveryâ€¦"). On loss, host baseline = N while peer baseline = N-1; since only changed fields are sent, the lost change is never re-sent. The `baselineTick` mismatch check exists but is **commented out**. Full snapshots only happen on player-list change.
...
**Binary grid encoding loses colors / mis-types garbage; opponent stacks render empty** â€” `binary-encoding.js:27-28, 357-387, 784-803`
*Refined diagnosis:* normal tetromino cells round-trip fine (`cell.type` = shapeKey, a valid letter). The real show-stopper: the renderer (`multi-player-canvas-layout.js:1572`) draws opponents from `lockedPieces` + currentPiece, but the encoder **strips lockedPieces** (writes count 0) and `_applySnapshotState` never reconstructs them from the decoded grid. Net in binary mode (production default): opponent **stacks render completely empty**, only the falling piece shows. Garbage cells also mis-encode: `'GARBAGE'`/`'CLEAN_GARBAGE'` shapeKeys aren't in `CELL_TYPE_MAP` (lowercase `'garbage'` only) â†’ decode as empty holes.
*Impact:* Opponent watch-boards appear empty; spectating is useless. Binary (production) only.
*Fix:* Make the opponent renderer grid-driven â€” reconstruct `lockedPieces` from the decoded grid in `_applySnapshotState`, or draw opponents directly from `gameState.grid`. Normalize garbage typing (lowercase the lookup, add `clean_garbage`). Reconstruct color on decode from canonical type (`COLORS[type]`/`COLORS.GARBAGE`). Add a test using a **real** board (`rebuildBoardGridFromPieces` + a garbage row) that asserts non-empty, correctly-typed/colored opponent render input.

**Host migration never restarts sim loop / jitter buffer / state sync** â€” `host-migration.js:110-144` (the active `src/core/network/` one)
`becomeHost()` flips flags, starts heartbeat, sends one snapshot â€” but never calls `startStateSyncLoop()` (no ongoing 30Hz broadcast), never creates the host-only `inputJitterBuffer`/`inputValidator` (null since the node started as a peer â†’ `validateInput` null-deref on first remote input), and never `syncUnifiedLoopPlayers()` (only the local player is registered, so remote gravity stops).
...
**Adaptive jitter buffer drops/skips a tick on depth change** â€” `input-jitter-buffer.js:187-209, 232-249, 360-377`
`_updateAdaptiveDepth()` mutates `bufferDepth` Â±1 at runtime; the process cursor is `currentTick - bufferDepth`. *Verified:* on **decrease** a tick is skipped and later GC'd as `inputsDropped` (real input loss); the **increase** case is a harmless 1-tick pause (not a double-apply, since the entry was already deleted). No per-tick guard, so multiple packets on a `%10` tick can shift depth by >1. Host-side online FFA only.
*Impact:* Under variable latency, occasional dropped remote inputs â€” pieces that "eat" a movement.
*Fix:* Track a separate monotonic `processCursor`; advance it by exactly 1 per `advanceTick`, and on a depth **decrease** drain the catch-up tick inline rather than abandoning it. Clamp depth changes to Â±1 per host tick. Add a unit test for Dâ†’DÂ±1 asserting every tick's inputs apply exactly once and `inputsDropped` stays 0.

**State sync on a drifting `setInterval`, not RAF-aligned** â€” `ffa-p2p-game-state.js:1393-1424, 2188-2204`
Host physics run in the RAF `onUpdate`, but broadcasts run on a separate `setInterval(1000/30)`, adding 0-33ms phase skew + timer coalescing. *Refined:* `hostTick` is **not** the authoritative input tick â€” the jitter buffer's own `currentTick` is already RAF-aligned, so the only genuine issue is snapshot phase skew.
...
**`lastInputSeq` never advanced for remote peers** â€” `ffa-p2p-game-state.js:965-970, 1524, 2213-2253`
...
**Resync re-serializes full match state to JSON+base64** â€” `ffa-p2p-game-state.js:1826-1872, 1890-1919`
...
**Interpolator mutates the cached snapshot object** â€” `snapshot-interpolation.js:190-196`
`_interpolate()` sets `toState.currentPiece = this._interpolatedPiece` (a reused shared object) on the **buffered** node. Next frame, that node's `currentPiece` now points at last frame's mutated object, collapsing the lerp endpoint toward A â€” opponent pieces under-shoot/stall then snap.
*Fix:* Never write to `toNode.data`. Return a per-steamId scratch wrapper: `out.currentPiece = interpolatedPiece; out.grid = toState.grid; â€¦; return out;` â€” keeps zero per-frame allocations while leaving the buffer immutable.

**Duplicate `GAME_ROUND_RESTART` key** â€” `message-types.js:41-42`
Declared twice (identical value); inert today but a trap for future edits.
*Fix:* Delete line 42; enable eslint `no-dupe-keys`.

### Audio

**No master SFX limiter/compressor â†’ clipping** â€” `sound-manager.js:303-333`
...
*Fix:* Create one shared `sfxBus` (Gain) â†’ `sfxLimiter` (`DynamicsCompressor`, threshold -3 to -6 dBFS, ratio ~12-20, attack ~0.003s, release ~0.1s) â†’ destination, once in `resumeAudioContext()`. Route `createTone`/`createRichTone` to `sfxBus`. Optionally run the music chain through a gentle limiter too.

**Move SFX retriggers every ~40ms DAS step** â€” `main.js:3254-3260`
`window.move` plays the move SFX on every successful shift; DAS repeats at `dasInterval=40ms`, but move tones are 50-100ms â€” so consecutive voices overlap into a buzzy drone during slides.
...
**Music HTMLAudio cold-start hitch** â€” `sound-manager.js:617-689`
...
**Dead `audioManager` singleton** â€” `audio-manager.js:359-372, 443-449`
`new AudioManager()` at import adds a permanent visibilitychange listener + window global; `theme-manager.js`'s `import { audioManager }` is shadowed by the injected SoundManager, and the `?.stopAll()` cleanup guard is a silent no-op (SoundManager has no `stopAll`). *Correction:* no idle AudioContext (lazy) â€” cost is one idle listener.
...
**`onStop`/restart never awaits in-flight physics** â€” `SinglePlayerMode.js:364-410` (+ Infinity `455-493`, Odyssey `410-462`)
`onStop()` sets `isGameOver`/cancels the loop but never awaits `latestPhysicsPromise`. A clear/cascade spans many frames; when it resolves, its `.then` unconditionally calls `spawnPiece()` against the (now-new) live `gameState` â€” phantom/double spawn. `DemoPlayer` already awaits `latestPhysicsPromise`, proving the correct pattern.
...
**`spawnPiece` has no isGameOver guard; top-out doesn't set the flag** â€” `game.js:369-441`
...
**Queued-input `setTimeout` bypasses pause/game-over guards** â€” `game.js:419-427`
...
*Impact:* Piece jumps/rotates one step right after pausing or game-over â€” a one-frame desync per piece that had a queued action.
...
## 4. Game-Feel Tuning Recommendations (best-practice values â†’ exact targets)
...
| **Soft drop (SDF)** | Gravity multiplier (e.g. 6x-20x) or configurable ms; allow âˆž (snap to floor, **non-locking**) | New `softDropInterval`/`softDropFactor`; replace hardcoded `50` in `controls.js:81/92` + gamepad |
...
- **JLSTZ** â€” `0â†’R:(0,0)(-1,0)(-1,+1)(0,-2)(-1,-2)`; `Râ†’0:(0,0)(+1,0)(+1,-1)(0,+2)(+1,+2)`; `Râ†’2:(0,0)(+1,0)(+1,-1)(0,+2)(+1,+2)`; `2â†’R:(0,0)(-1,0)(-1,+1)(0,-2)(-1,-2)`; `2â†’L:(0,0)(+1,0)(+1,+1)(0,-2)(+1,-2)`; `Lâ†’2:(0,0)(-1,0)(-1,-1)(0,+2)(-1,+2)`; `Lâ†’0:(0,0)(-1,0)(-1,-1)(0,+2)(-1,+2)`; `0â†’L:(0,0)(+1,0)(+1,+1)(0,-2)(+1,-2)`.
- **I-piece (separate table)** â€” `0â†’R:(0,0)(-2,0)(+1,0)(-2,-1)(+1,+2)`; `Râ†’0:(0,0)(+2,0)(-1,0)(+2,+1)(-1,-2)`; `Râ†’2:(0,0)(-1,0)(+2,0)(-1,+2)(+2,-1)`; `2â†’R:(0,0)(+1,0)(-2,0)(+1,-2)(-2,+1)`; `2â†’L:(0,0)(+2,0)(-1,0)(+2,+1)(-1,-2)`; `Lâ†’2:(0,0)(-2,0)(+1,0)(-2,-1)(+1,+2)`; `Lâ†’0:(0,0)(+1,0)(-2,0)(+1,-2)(-2,+1)`; `0â†’L:(0,0)(-1,0)(+2,0)(-1,+2)(+2,-1)`.
...
- Add `.catch` to the physics promise (`game.js:722-731`) â€” prevents the worst freeze. **(small, do first)**
...
- Peer board overwrite guard â€” Option A (`ffa-p2p-game-state.js`).
...
- **Lock delay + move reset** state machine (`game.js`) â€” medium, touches multiplayer/demos; deploy with deterministic delta.
- **Full SRS rotation + kick tables (+ optional T-spin)** (`game.js`, `pieces.js`) â€” large; add orientation field first; invalidates replays, deploy atomically.
...
- **Binary opponent-board encoding** â€” reconstruct `lockedPieces`/grid + colors + garbage typing (`binary-encoding.js`, `ffa-p2p-game-state.js`) + a real-board test.
- **Multiplayer netcode hardening** â€” baseline-tick reject + periodic keyframe (`steam-networking.js`), host-migration `promoteToHost()`, jitter-buffer `processCursor` decoupling. Defer client-side prediction (`inputHistory`/`lastInputSeq`) unless actively completing it.
