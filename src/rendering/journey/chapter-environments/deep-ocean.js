...
void main() {
    // Vertical fade (fade out at bottom)
    float fade = 1.0 - smoothstep(0.0, 1.0, vUv.y);
    
    // Animated caustic beams
    float beam = sin(vPos.x * 0.5 + uTime) * sin(vPos.z * 0.5 + uTime * 0.7);
    beam = smoothstep(0.0, 1.0, beam * 0.5 + 0.5);
    
    // Side fade
    float sideFade = 1.0 - pow(abs(vUv.x - 0.5) * 2.0, 4.0);
    
    float alpha = fade * beam * sideFade * 0.3;
...
export function createDeepOceanEnvironment(options = {}) {
    ...
    const waterSurface = createWaterSurface(uniforms);
    ...
    const jellyfishCount = Math.floor((options.particleCount || 500) / 10);
    ...
    group.position.y = (DEEP_OCEAN_CONFIG.yStart + DEEP_OCEAN_CONFIG.yEnd) / 2;
...
function createWaterSurface(uniforms) {
...
function createGodRays(uniforms) {
    const group = new THREE.Group();
    // Use cones strictly pointing down
    const geometry = new THREE.CylinderGeometry(2, 10, 80, 16, 4, true);
    geometry.translate(0, -40, 0); // Pivot at top
...
function createBioluminescentJellyfish(uniforms, count) {
    ...
    // Simple glowing sphere for jelly body
    const geo = new THREE.SphereGeometry(1, 16, 16);
    const mat = new THREE.MeshBasicMaterial({
        color: 0x00ffff,
        transparent: true,
        opacity: 0.6,
        wireframe: true // Looks techy/cool
    });
    ...
    for (let i = 0; i < count; i++) {
        ...
        mesh.position.set(
            (Math.random() - 0.5) * 100,
            (Math.random() - 0.5) * 30, // Spread vertically
            -20 - Math.random() * 60
        );
...
export function updateDeepOceanEnvironment(group, delta, time) {
    ...
    if (jellies) {
        jellies.children.forEach(j => {
            j.position.y += Math.sin(time + j.userData.t) * delta;
            j.position.x += Math.cos(time * 0.5 + j.userData.t) * delta;
        });
    }
