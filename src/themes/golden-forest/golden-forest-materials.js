/**
 * Golden Forest Theme - WebGPU TSL node materials (Phase 2 sky system)
 */
...
export function createAuroraNodeMaterial(params = {}) {
    const uTime = uniform(params.time ?? 0);
    const uIntensity = uniform(params.intensity ?? 0.85);
    const uColor1 = uniform(params.color1 ?? new THREE.Color(0xFF9A4D));
    const uColor2 = uniform(params.color2 ?? new THREE.Color(0xFFD18A));
    const uColor3 = uniform(params.color3 ?? new THREE.Color(0xE96D36));
    const uOffset = uniform(params.offset ?? 0);

    const uvNode = uv();
    const wave1 = sin(uvNode.x.mul(4.0).add(uTime.mul(0.3)).add(uOffset)).mul(0.3);
    const wave2 = sin(
        uvNode.x.mul(6.0).sub(uTime.mul(0.2)).add(uOffset.mul(1.5)),
    ).mul(0.2);
    const wave3 = cos(
        uvNode.x.mul(3.0).add(uTime.mul(0.4)).add(uOffset.mul(0.8)),
    ).mul(0.25);
    const yPos = uvNode.y.add(wave1.add(wave2).add(wave3).mul(0.3));
    const lowerFade = smoothstep(float(0.3), float(0.5), yPos);
    const upperFade = float(1.0).sub(smoothstep(float(0.6), float(0.9), yPos));
    const band = lowerFade.mul(upperFade);
    const flowNoise = noise2D(
        uvNode.mul(2.0).add(vec2(
            uTime.mul(0.15).add(uOffset),
            uTime.mul(-0.07).add(uOffset.mul(0.37)),
        )),
    ).mul(2.0).sub(1.0);
    const shapedBand = band.mul(flowNoise.mul(0.3).add(0.7));

    const sideColor = mix(uColor1, uColor2, uvNode.x);
    const color = mix(sideColor, uColor3, float(1.0).sub(uvNode.y).mul(0.5));
    const alpha = clamp(shapedBand.mul(uIntensity).mul(0.5), 0.0, 1.0);
...
/**
 * Golden Koi-grade lake water (WebGPU/TSL).
 *
 * Ports the Koi Pond v2 surface optics — an analytic layered-wave *optical
 * normal* (three directional sines + one FBM detail octave) that actually bends
 * the light, Fresnel-mixed depth transmission ↔ warm-sky reflection, and a
 * broad+sparkle specular glint lane — retuned golden for the theme's low golden
 * sun. Unlike createWaterNodeMaterial() (a re-skin of the old WebGL Water.js),
 * every highlight here rides the wave-derived normal, so the lake reads as
 * water, not a painted gradient.
 *
 * Coordinate contract: the lake mesh is rotated -PI/2 about X, so
 * positionWorld.xz is the horizontal plane and positionLocal.z is world-up.
 * Waves are sampled from positionWorld.xz; height is displaced on positionLocal.z.
 *
 * Deliberately analytic (no viewportTexture refraction, no reflector() pass):
 * the SF lakebed isn't modelled, the lake spans ~±100 units, and a full-screen
 * copy inside the theme's post pipeline is the fragile/expensive path. A real
 * planar reflector is the documented upgrade if wanted later.
 *
 * Returns { material, uniforms } with uTime + uSunDirection kept API-compatible
 * with the theme's per-frame wiring, plus golden tunables.
 */
export function createGoldenLakeNodeMaterial(params = {}) {
    const uTime = uniform(params.time ?? 0);
    const uSunDirection = uniform(
        (params.sunDirection ?? new THREE.Vector3(0.0, 0.2, -1.0)).clone().normalize(),
    );
    const uSunColor = uniform(params.sunColor ?? new THREE.Color(0xffc266));
    // Water BODY: a warm amber-brown (not near-black) so the lake glows in the
    // golden light instead of reading as mud where the mirror is dark.
    const uDeepColor = uniform(params.deepColor ?? new THREE.Color(0x2c1608));
    const uShallowColor = uniform(params.shallowColor ?? new THREE.Color(0x5a3016));
    // Reflected sky gradient (warm): bright horizon glow near the waterline
    // fading to a deeper amber overhead reflected in the nearer water.
    const uHorizonColor = uniform(params.horizonColor ?? new THREE.Color(0xffb45a));
    const uUpperColor = uniform(params.upperColor ?? new THREE.Color(0x9a4418));
    // Reflected far-shore treeline — near-black warm, matching the silhouettes.
    const uTreelineColor = uniform(params.treelineColor ?? new THREE.Color(0x160b04));
    const uGlintColor = uniform(params.glintColor ?? new THREE.Color(1.0, 0.88, 0.58));
    const uWaveScale = uniform(params.waveScale ?? 1.0); // multiplies wave frequency
    // Small lake ripples: a gentle vertical texture, NOT ocean swells. Too much
    // here and tall crests show the dark body and read as brown dunes/ground.
    const uWaveHeight = uniform(params.waveHeight ?? 0.22); // world-up displacement amplitude
    // Most of the "moving water" look lives in the NORMAL (fine dense ripples +
    // FBM detail) and the dancing glints, not in tall geometry.
    const uNormalStrength = uniform(params.normalStrength ?? 1.9);
    const uDetailStrength = uniform(params.detailStrength ?? 0.55);
    const uSunTrackStrength = uniform(params.sunTrackStrength ?? 0.85);
    const uSunLaneWidth = uniform(params.sunLaneWidth ?? 30.0); // world half-width of the sun column
    const uGlitterStrength = uniform(params.glitterStrength ?? 0.55); // broad sparkle across the lake
    const uSheenStrength = uniform(params.sheenStrength ?? 0.14); // warm grazing sheen
    const uReflectionStrength = uniform(params.reflectionStrength ?? 0.9);
    const uReflectionDistort = uniform(params.reflectionDistort ?? 0.052); // wave-ripple bend of the mirror
    const uTreelineStrength = uniform(params.treelineStrength ?? 0.85); // how darkly the far shore mirrors (fallback)
    const uShoreDarkening = uniform(params.shoreDarkening ?? 0.4);
    const uShoreFoamStrength = uniform(params.shoreFoamStrength ?? 0.08);
    const uObjectFoamStrength = uniform(params.objectFoamStrength ?? 0.12);
    const uEmissiveStrength = uniform(params.emissiveStrength ?? 0.4);

    const uvNode = uv();
    const worldXZ = vec2(positionWorld.x, positionWorld.z);

    // ── Analytic wave field → world-space optical normal ──────────────────
    // Three directional sine layers give slow rolling swells; the analytic
    // slope of each becomes the surface tilt (no texture normal). Amplitudes
    // are unitless shape weights (sum ~1); overall height is scaled by uWaveHeight.
    // Small, dense lake ripples from three crossing directions (no single
    // dominant swell). Higher frequencies = shorter wavelengths (~10-30u) so the
    // surface reads as fine lake texture rather than big rolling crests.
    const dirA = vec2(0.30, 0.95);
    const dirB = vec2(-0.62, 0.78);
    const dirC = vec2(0.90, 0.44);
    const freqA = float(0.130).mul(uWaveScale);
    const freqB = float(0.260).mul(uWaveScale);
    const freqC = float(0.470).mul(uWaveScale);
    const ampA = float(0.48);
    const ampB = float(0.33);
    const ampC = float(0.22);

    const phaseA = dot(worldXZ, dirA).mul(freqA).add(uTime.mul(0.5));
    const phaseB = dot(worldXZ, dirB).mul(freqB).sub(uTime.mul(0.66));
    const phaseC = dot(worldXZ, dirC).mul(freqC).add(uTime.mul(0.95));

    const height = sin(phaseA).mul(ampA)
        .add(sin(phaseB).mul(ampB))
        .add(sin(phaseC).mul(ampC));

    const derivA = cos(phaseA).mul(ampA).mul(freqA);
    const derivB = cos(phaseB).mul(ampB).mul(freqB);
    const derivC = cos(phaseC).mul(ampC).mul(freqC);
    let slopeX = derivA.mul(dirA.x).add(derivB.mul(dirB.x)).add(derivC.mul(dirC.x));
    let slopeZ = derivA.mul(dirA.y).add(derivB.mul(dirB.y)).add(derivC.mul(dirC.y));

    // One FBM detail octave: a cheap forward-difference gradient of drift noise
    // perturbs only the OPTICAL slope (not vertex height), so the mirror and
    // sun glint scintillate organically instead of via an authored sin band.
    const detailScale = float(0.42).mul(uWaveScale);
    const detailDrift = vec2(uTime.mul(0.11), uTime.mul(-0.08));
    const dBase = worldXZ.mul(detailScale).add(detailDrift);
    const n0 = noise2D(dBase);
    const nx = noise2D(dBase.add(vec2(0.18, 0.0)));
    const nz = noise2D(dBase.add(vec2(0.0, 0.18)));
    slopeX = slopeX.add(nx.sub(n0).mul(uDetailStrength));
    slopeZ = slopeZ.add(nz.sub(n0).mul(uDetailStrength));

    const opticalNormal = normalize(vec3(
        slopeX.mul(uNormalStrength.negate()),
        1.0,
        slopeZ.mul(uNormalStrength.negate()),
    ));

    // ── View / Fresnel ────────────────────────────────────────────────────
    const viewDir = normalize(cameraPosition.sub(positionWorld));
    const viewFacing = max(dot(opticalNormal, viewDir), float(0.0));
    // A little base reflectivity keeps the near foreground alive (it still
    // catches warm sky), while grazing far water goes almost fully to the mirror.
    const fresnel = float(0.08).add(
        float(0.92).mul(pow(float(1.0).sub(viewFacing), float(5.0))),
    );

    const centerDist = length(uvNode.sub(0.5)).mul(2.0);
    const edgeInset = float(1.0).sub(smoothstep(float(0.72), float(0.995), centerDist));
    const depthFactor = smoothstep(float(0.30), float(1.0), centerDist).oneMinus(); // 1 deep centre

    // ── Dark water body ────────────────────────────────────────────────────
    const waterBody = mix(uShallowColor, uDeepColor, pow(depthFactor, float(0.7)));

    // ── Reflected sun column + glint (compute first; the trees part for it) ─
    const sunDir = normalize(uSunDirection);
    const halfVec = normalize(viewDir.add(sunDir));
    const normalHighlight = max(dot(opticalNormal, halfVec), float(0.0));
    const broadSpecular = pow(normalHighlight, float(30.0));
    const sparkleSpecular = pow(normalHighlight, float(120.0));
    const glintBreakup = pow(
        abs(sin(worldXZ.x.mul(0.28).add(worldXZ.y.mul(0.52)).sub(uTime.mul(0.31)))),
        float(8.0),
    );
    // Sun sits straight down -Z (world X ~ 0), so the column centres on X=0.
    const sunLane = float(1.0).sub(smoothstep(float(0.6), uSunLaneWidth, abs(worldXZ.x)));
    // A soft, wide reflected-sun glow (the warm smear around the column) and the
    // crisp shimmering track on top of it.
    const sunGlow = pow(normalHighlight, float(3.5)).mul(sunLane);
    const sunTrack = broadSpecular.mul(0.34)
        .add(sparkleSpecular.mul(glintBreakup.mul(0.75).add(0.25)))
        .mul(sunLane);
    const sunPresence = clamp(sunGlow.mul(1.3).add(sunTrack), 0.0, 1.0);

    // ── Reflection: a real planar mirror when supplied, else procedural ────
    const reflection = params.reflection ?? null;
    let reflected;
    if (reflection) {
        // Sample the mirrored scene (dark trees + warm sky + sun) rippled by the
        // wave normal. screenUV.flipX() matches the reflector's stored UV.
        const reflDistort = vec2(opticalNormal.x, opticalNormal.z)
            .mul(uReflectionDistort)
            .mul(edgeInset.mul(0.7).add(0.3));
        const reflUv = clamp(
            screenUV.flipX().add(reflDistort),
            vec2(0.003),
            vec2(0.997),
        );
        const mirror = reflection.sample(reflUv).rgb;
        // Water warms + lifts what it mirrors (the surface glows in the golden
        // light); a warm floor keeps deep shade between trees from dead black.
        reflected = mirror.mul(vec3(1.16, 1.0, 0.78)).add(uHorizonColor.mul(0.08));
    } else {
        // Procedural fallback (no reflector): warm sky gradient + a dark
        // treeline occlusion band in the grazing far strip. Keep the bright
        // horizon across far-AND-mid water; only the nearest water deepens.
        const rippleTilt = opticalNormal.x.add(opticalNormal.z);
        const skyMix = clamp(
            smoothstep(float(0.24), float(0.62), viewFacing).add(rippleTilt.mul(0.10)),
            0.0,
            1.0,
        );
        const sky = mix(uHorizonColor, uUpperColor, skyMix);
        // Broad band: tall trees on the far shore reflect a long way toward the
        // camera; a jagged pine silhouette, ripple-broken, sun punches through.
        const farStrip = smoothstep(float(0.05), float(0.30), viewFacing).oneMinus();
        const pineCoarse = noise2D(vec2(
            worldXZ.x.mul(0.22),
            worldXZ.y.mul(0.05).add(uTime.mul(0.02)),
        ));
        const pineFine = abs(sin(worldXZ.x.mul(0.9).add(pineCoarse.mul(3.0))));
        const pine = clamp(pineCoarse.mul(0.55).add(pineFine.mul(0.55)).add(0.15), 0.0, 1.0);
        const bandWobble = sin(worldXZ.x.mul(0.6).add(uTime.mul(0.4))).mul(0.08).add(0.92);
        const treeline = clamp(
            farStrip.mul(pine).mul(bandWobble).mul(uTreelineStrength)
                .mul(float(1.0).sub(sunPresence)),
            0.0,
            1.0,
        );
        reflected = mix(sky, uTreelineColor, treeline);
    }
    // Warm horizon glow bleeding up toward the sun rides on top either way.
    reflected = reflected.add(uHorizonColor.mul(sunGlow.mul(0.28)));

    // ── Compose: dark water body ← mirror, weighted by Fresnel ─────────────
    const reflectance = clamp(fresnel, 0.0, 0.94).mul(uReflectionStrength);
    let waterColor = mix(waterBody, reflected, reflectance);

    // Bright reflected-sun sparkle rides on top of everything.
    const sunTrackColor = uGlintColor.mul(sunTrack.mul(uSunTrackStrength));
    waterColor = waterColor.add(sunTrackColor);

    // Broad "floating water" glitter: any wave facet that catches the low sun
    // sparkles, densest toward the sun but not lane-locked. This is what sells
    // the surface as a moving liquid rather than a still dark mirror.
    const skyGlitter = pow(normalHighlight, float(64.0)).mul(uGlitterStrength);
    waterColor = waterColor.add(uGlintColor.mul(skyGlitter));
    // Warm grazing sheen so the whole surface glows amber instead of muddy.
    const sheen = pow(float(1.0).sub(viewFacing), float(3.0)).mul(uSheenStrength);
    waterColor = waterColor.add(uHorizonColor.mul(sheen));

    // ── Shore vignette + subtle foam ───────────────────────────────────────
    const shoreShadow = smoothstep(float(0.86), float(1.0), centerDist);
    waterColor = mix(waterColor, vec3(0.10, 0.05, 0.02), shoreShadow.mul(uShoreDarkening));
    ...
    const shoreFoamB = sin(worldXZ.x.mul(1.5).sub(uTime.mul(0.8)))
        .mul(sin(worldXZ.y.mul(1.2).add(uTime.mul(1.1))));
    const shorePattern = shoreFoamA.add(shoreFoamB.mul(0.6)).mul(0.5).add(0.5);
    let shoreFoam = smoothstep(float(0.3), float(0.7), shorePattern).mul(shoreFoamZone);
    shoreFoam = shoreFoam.mul(uShoreFoamStrength);
    ...
    const foam4 = smoothstep(float(12.0), float(3.6), length(worldXZ.sub(vec2(85.0, 5.0))));
    const foam5 = smoothstep(float(11.0), float(3.2), length(worldXZ.sub(vec2(100.0, 12.0))));
    let objectFoam = max(max(max(foam1, foam2), max(foam3, foam4)), foam5);
    const objectFoamWave = sin(worldXZ.x.mul(0.8).add(uTime.mul(1.0)))
        .mul(sin(worldXZ.y.mul(0.6).add(uTime.mul(0.8))))
        .mul(0.25)
        .add(0.75);
    objectFoam = objectFoam.mul(objectFoamWave).mul(uObjectFoamStrength);
    const foamColor = vec3(0.95, 0.78, 0.55);
    waterColor = mix(waterColor, foamColor, max(shoreFoam, objectFoam));

    // ── Output ─────────────────────────────────────────────────────────────
    const edgeAlpha = float(1.0).sub(smoothstep(float(0.97), float(1.0), centerDist));
    const waveDisplacement = height.mul(uWaveHeight).mul(edgeInset.mul(0.85).add(0.15));
    ...
    material.opacityNode = edgeAlpha.mul(0.985);
    // Only the light the surface MAKES (the reflected sun sparkle + its glow)
    // feeds the theme bloom — the dark body and mirror stay out of it.
    material.emissiveNode = sunTrackColor
        .add(uGlintColor.mul(skyGlitter.mul(0.6)))
        .add(uGlintColor.mul(sunGlow.mul(0.12)))
        .mul(uEmissiveStrength);
    // Lake mesh is rotated -PI/2 about X, so local Z is world-up.
    material.positionNode = vec3(
        positionLocal.x,
        positionLocal.y,
        positionLocal.z.add(waveDisplacement),
    );
