...
export class GoldenForestBirds {
    ...
    initWebGPUBirds() {
        if (typeof this.renderer.compute !== 'function') {
            console.warn('[GoldenForestBirds] WebGPU renderer has no compute() API; skipping bird compute init.');
            return;
        }

        this.birdCompute = new GoldenForestBirdCompute(this.BIRDS, this.randomFn);
