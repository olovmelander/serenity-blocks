...
export default class PyrestormTheme extends BaseTheme {
    ...
    createSurroundings() {
        ...
        const horizonGeo = new THREE.CylinderGeometry(12000, 12000, 2500, 32, 1, true);
        const pos = horizonGeo.attributes.position;
        // Displace top edge to look like mountains
        for (let i = 0; i < pos.count; i++) {
            if (pos.getY(i) > 0) { // Top vertices
                const x = pos.getX(i);
                const z = pos.getZ(i);
                const angle = Math.atan2(z, x);
                const noise = Math.sin(angle * 12) * Math.cos(angle * 35) * 600;
                pos.setY(i, 1000 + noise + Math.random() * 200);
            } else {
        ...
        const horizonMat = new THREE.ShaderMaterial({
            uniforms: {
                topColor: { value: new THREE.Color(0x802010) }, // Bright Magma Red Top
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    gl_FragColor = vec4(mix(bottomColor, topColor, smoothstep(0.0, 0.7, vUv.y)), 1.0);
                }
            `,
