/**
 * Himalayan Peak AAA — Camera Director
 *
 * Cinematic camera choreography for the valley-looking-up-at-the-massif framing.
 * Same proven spring/shake/FOV-punch core as Electric Dreams V3's CameraDirector,
 * retuned for the mountain scene's larger world scale.
 *
 *   Idle:        slow figure-8 drift around the rest pose ("breathing")
 *   PIECE_LOCK:  micro-shake + tiny FOV punch
 *   HARD_DROP:   shake + dolly nudge
 *   LINE_CLEAR / COMBO: dolly-push + FOV pulse (driven via director.cameraPunch)
 *   GAME_OVER:   slow pull-back to a wide shot
 *
 * Constraints: clamp displacement + FOV delta so the board never disorients.
 * See docs/HIMALAYAN_PEAK_AAA_PLAN.md §2, §5.
 */
import * as THREE from 'three';

// World scale: peaks ~250u tall, ranges recede to ~-1000u. Camera sits low in
// the valley looking up and into the distance.
const DEFAULT_FOCUS = new THREE.Vector3(0, 95, -260);
const REST_POSITION = new THREE.Vector3(0, 58, 160);

const SPRING_FREQ = 1.1; // Hz — slow, weighty
const SPRING_DAMP = 1.0; // critically damped
const MAX_FOV_DELTA = 5; // degrees
const MAX_OFFSET_LEN = 14; // world units (scaled up for the big scene)

// Pointer parallax — subtle; the board lives in screen-center.
...
// Idle figure-8 (Lissajous) — long periods, gentle amplitude.
...
export class CameraDirector {
     ...
     * a dolly-push toward the massif + FOV pull, scaled by strength.
     */
