...
export default class FloatingIslandsTheme extends BaseTheme {
    ...
    drawIsland(ctx, island) {
        ...
        for(let i=0; i<width/8; i++) {
            ...
            ctx.bezierCurveTo(rootStartX + this.random(-20, 20), rootStartY + 30, rootStartX + this.random(-10, 10), rootStartY + 60, rootStartX + this.random(-5, 5), rootStartY + 90);
            ctx.stroke();
        }
        ...
        ctx.fill();
        if(island.tree) {
            this.drawMajesticTree(ctx, x + width / 2, topSurface[Math.floor(width/2)].y, island.tree.height);
        }
