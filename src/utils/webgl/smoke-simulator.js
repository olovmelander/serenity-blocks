 ...
 * - Buoyancy force (smoke rises naturally)
 * - Optimized for grayscale/colored smoke visuals
 */

export default class SmokeSimulator {
    ...
    initPrograms() {
        ...
        // SMOKE-SPECIFIC: Buoyancy shader for rising smoke
        const buoyancyShaderSource = `
            ...
            void main () {
                ...
                // Apply buoyancy force (smoke rises)
                velocity.y += buoyancy * density * dt;
        ...
        this.programs.buoyancy = new Program(gl, baseVertexShader, this.compileShader(gl.FRAGMENT_SHADER, buoyancyShaderSource));
    ...
    step(dt) {
        ...
        // SMOKE-SPECIFIC: Apply buoyancy force (smoke rises)
        if (config.BUOYANCY > 0) {
            this.programs.buoyancy.bind();
            gl.uniform1i(this.programs.buoyancy.uniforms.uVelocity, this.velocity.read.attach(0));
            gl.uniform1i(this.programs.buoyancy.uniforms.uDensity, this.dye.read.attach(1));
            gl.uniform1f(this.programs.buoyancy.uniforms.buoyancy, config.BUOYANCY);
            gl.uniform1f(this.programs.buoyancy.uniforms.dt, dt);
            this.blit(this.velocity.write);
            this.velocity.swap();
        }
