...
export function createSoundSets(createTone) {
    return {
        ...
        Nebula: {
            ...
            drop: () => {
                [240, 180].forEach((f, i) => setTimeout(() => createTone(f, 0.28, 'sine', 0.2), i * 90));
            },
            lineClear: () => {
            ...
            levelUp: () => {
                [330, 494, 660, 880].forEach((f, i) => setTimeout(() => createTone(f, 0.42, 'triangle', 0.22), i * 120));
                setTimeout(() => createTone(1175, 0.48, 'sine', 0.18), 520);
            },
            gameOver: () => {
                [523, 392, 261, 196].forEach((f, i) => setTimeout(() => createTone(f, 0.5, 'sine', 0.17), i * 210));
                setTimeout(() => createTone(130, 0.7, 'triangle', 0.22), 860);
            },
            garbageSend: () => {
                // Cosmic whoosh with echo
                [700, 1100, 1500].forEach((f, i) => setTimeout(() => createTone(f, 0.15, 'triangle', 0.2), i * 35));
