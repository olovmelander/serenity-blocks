...
> **Status**: IN PROGRESS. **Phases 0–3 + P3b SHIPPED**; **P4 SHIPPED** (P4a environment anchors 2026-06-02; **P4b quality pass 2026-06-04** — shader accretion disks/lensing, FBM nebula domes, neon procedural lit-window facades; focused Odyssey lint clean, 29 targeted Odyssey tests green, production build green). Art direction below is still a proposal; transition designs (§6) should still be locked with the user before Phase 5. **Remaining for P4:** WebGPU/visual capture on Windows Chrome to confirm the look.
>
...
>
> **P4a shipped** — thin-chapter environment-anchor pass, gated by normal chapter loading:
> - **Ch5 Sky & Drift** (`sky-drift.js`): added a cloud-deck break, animated light shaft, aurora ribbons, and rain veils layered into the existing galaxy/planet backdrop.
> - **Ch6 Space** (`cosmic-expanse.js`): replaced the flat black-hole plane with a 3D volumetric black-hole anchor (sphere horizon + torus accretion bands + lens shell), plus a hero planet and nebula-volume points.
> - **Ch7 Black Hole** (`black-hole-transcendence.js`): upgraded the event horizon to a dominant 3D anchor with photon/accretion rings, lensing starfield, and infall streams.
> - **Ch8 Urban Encore** (`urban-dreams.js`): added a neon megastructure spire, hologram signs, wet reflection plane, and sky-traffic light trails.
> - Focused lint clean; new `odyssey-p4-environments.test.js` verifies the P4 anchor layers instantiate; 28 targeted Odyssey tests green. Visual capture still pending on a working browser/Electron capture path.
>
> **P4b shipped** — thin-chapter **quality pass** (2026-06-04), bringing the four thin worlds up to the rich-chapter shader bar; always-on (not flag-gated):
> - **Shared GLSL noise** (`chapter-environments/shared/odyssey-noise.js`): hash / value-noise / `fbm2` / `fbm3` / `ridged3` / `od_curl3` chunks the chapter shaders concatenate, so all worlds share one noise vocabulary instead of re-deriving their own.
> - **Ch6 Space** (`cosmic-expanse.js`): the flat additive tori became a **shader accretion disk** (FBM plasma + Keplerian differential rotation + Doppler beaming); the near-black void became an **FBM nebula dome**; the planet a banded gas giant (storm bands + day/night terminator + atmosphere rim); plus a twinkle-shader starfield and a fresnel gravitational-lensing shell.
> - **Ch7 Black Hole** (`black-hole-transcendence.js`): dominant event horizon with the shader accretion disk (hotter magenta/cyan palette), an **Einstein-ring fresnel lensing shell**, an FBM nebula dome, and twinkle-shader lensed starfield + shards.
> - **Ch8 Urban Encore** (`urban-dreams.js`): the headline fix — a **procedural lit-window facade shader** (randomly-lit, flickering window grids) replacing the flat translucent window-planes; a neon sky with city-glow + drifting smog; an animated hologram-sign shader; an FBM wet-reflection plane; rain as vertical streaks; ground neon haze; an energy-conduit spire core.
> - **Ch5 Sky & Drift** (`sky-drift.js`): lighter polish (already the richest of the four) — an FBM **volumetric nebula veil** for backdrop depth + a `uEnergy` reactivity hook.
> - All four self-animate a `uEnergy` "breath" uniform, structured so **Phase 6** can drive it from the audio reactor with no shader changes.
> - **Hardening (adversarial multi-lens review):** added spline-aligned `yStart`/`yEnd` to the Ch7/Ch8 configs (every other chapter declared them; `ChapterEnvironmentManager.getChapterAtPosition` reads `config.yStart/yEnd`) and made `create()` always populate `userData.yStart/yEnd` with a config fallback; locked by a new config-range test.
> - **Validation:** focused Odyssey lint clean; **29 targeted Odyssey tests green**; production `vite build` green. WebGPU/visual capture on Windows Chrome still pending (per the WSL/Electron constraint — the shaders are shipped as strings and are not compiled by tests/build).
>
...
| **P4 — Level up the 4 thin chapters** ✅ **P4a + P4b SHIPPED** | Anchors (P4a) **+ full shader quality pass (P4b)**: Ch6 shader accretion disk + FBM nebula dome ✓, Ch7 Einstein-ring lensing + accretion ✓, Ch5 FBM nebula veil + reactivity ✓, Ch8 procedural lit-window facades + neon city ✓; shared `odyssey-noise.js` GLSL vocabulary. | `cosmic-expanse.js`, `black-hole-transcendence.js`, `sky-drift.js`, `urban-dreams.js`, `shared/odyssey-noise.js` | Focused lint clean, 29 targeted Odyssey tests green, production build green, adversarial multi-lens review (findings fixed). Remaining: WebGPU/visual capture on Windows Chrome to confirm the look (per WSL/Electron constraint). |
...
- [x] `chapter-environments/shared/odyssey-noise.js` — shared hash/value/fbm/ridged/curl GLSL — **P4b**
...
- [~] `mountain-peaks.js` 🔴 hero summit/alpenglow • [x] `sky-drift.js` ✅ P4a weather + **P4b** FBM nebula veil + reactivity
- [x] `cosmic-expanse.js` ✅ **P4b** shader accretion disk + FBM nebula dome + gas giant • `black-hole-transcendence.js` ✅ **P4b** Einstein-ring lensing + accretion disk • `urban-dreams.js` ✅ **P4b** procedural lit-window facades + neon city
