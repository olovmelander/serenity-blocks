...
- [x] **Wave 6** — Transitions become occlusion — **CLOSED AS RESCOPED (2026-08-12). The deletion premise is dead; the constructive half is promoted to its own item below.** Nothing further is deletable: 34 of the ~47 bridges belong to chapters 1/7/8 which still draw, and Ch2's 11 fallback-only ones are now KEPT on purpose (ADR-0015 — deleting them degrades the crash-recovery path we chose to keep, so they stopped being free). What WAS dead went in Tranche 1: the `ECOTONE_*` scaling arithmetic and the manager-side 3-4/4-5 colour windows. **"Delete the ecotone machinery" would have broken three LIVE chapters:** Same 13-agent audit as Wave 4; both verification lenses refuted the "fallback-only" claim. The corrections:
...
- [ ] **Occlusion moments** (promoted out of Wave 6, 2026-08-12) — **ch1->Act II SHIPPED
  ...
  **STILL TO DO — the ch5->ch6 cloud bank**, and it is the harder of the two: the one piece of
  ...
  describes at §"cloud banks that conform to terrain … this IS the Ch5->Ch6 occlusion moment").
  Build those two as traversable occlusion — fog as an object you fly through, not a post
  effect — and the crossfade at each becomes redundant by construction.
  **Sequencing rule, learned the hard way: BUILD FIRST.** Deleting the bridges first leaves the
  two real seams with nothing, and the audit already showed the deletion cannot proceed anyway.
  Owes a capture at each edge (ADR-0007), and the 5->6 edge is where `SEAM_56_AURORA_BRIDGE`
  is visibly load-bearing today — that bridge is what the occlusion moment must replace, not
  something to remove alongside it.
