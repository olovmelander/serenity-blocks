...
- [ ] **Wave 7** — Perf, tiers, residency, rail furniture (15 %: not started, but no longer blind — the split harness now seeks into Act II and compares against the legacy fallback, and One World measures **1.57 ms cheaper than the dioramas at zero drift**. The Lane B re-measure is the next real step: the iGPU was 67.7 ms against a 7.0 ms budget BEFORE this saving, so the question is now how much of that 10x gap the rebuild closes on its own.)
