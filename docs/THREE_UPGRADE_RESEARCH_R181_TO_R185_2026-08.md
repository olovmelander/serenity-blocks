...
| **CORRECTED 2026-08-21 — was filed here as "NEW r185 bug: `compileAsync(scene, camera, group)` TypeErrors"**. It was the repo's misuse: three's contract is `compileAsync(objectToCompile, camera, targetScene)` and the call was inverted since r181 — every "targeted" prewarm walked the WHOLE scene and read `background` off a Group. r181 tolerated it (sync build, cache hits); r185's per-object yielding loop made four whole-scene walks cost seconds (the tell: corridor/breach/one-world "compiles" of identical length regardless of group size) | Every targeted compile in the repo | Fixed: `compileAsync(group, camera, scene)` in `compileGroupThroughPost`; the `group.background = null` patch + test removed; parameter order pinned from the installed source by `odyssey-post-target-compile.test.js`. Upstream Issue 1 **withdrawn** in `docs/UPSTREAM_THREE_R185_ISSUES_READY_TO_FILE.md` |
