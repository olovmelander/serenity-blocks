...
pipeline (item 2.9), which the r185 ledger's window never reached. Content match holds
(80 / 104 draws, 255 k / 538 k triangles in all three trees).
...
| 2.9 | **Background chapter creation invalidates every visible pipeline (the QW4 rig meets deferred loading).** Measured with the session's sync-creation hook + a light snapshot: at ~10 s chapter 6 is created in the background and its lights (intensity 0, **visible**) are reparented onto the persistent light rig; `lightsNode.getCacheKey()` is part of *every* material's builder key (lit or not), so all 41–42 visible pipelines are rebuilt and re-created synchronously on the next frame (2.2 s cold); chapters 7 and 8 repeat it (33–38 and 8–11 pipelines at 13–14 s). The rig was designed to keep the light SET constant across seams, which it does — but with chapters 3–8 created after the reveal the set now grows three times right after the board appears, and the prewarm can only compile against the set that exists at load. The r185 ledger's load cells never reached chapter 6's creation in their window, which is why this read as "new". It is also the first-visit hitch the July plan §7 measured. Ruled out on the way (each by experiment): render-context call depth, attachment shape, MRT, the prewarm target. Fix candidates: (a) a per-chapter **light manifest** so the rig is pre-seeded with the union light set at startup and `_reparentChapterLights` adopts pre-seeded lights (set constant from the first prewarm); (b) create far chapters' **lights** (not their meshes) before the startup prewarm; (c) front-load chapter creation before the prewarm on machines where the ~1 s is affordable. (a) is the right shape. | removes the three 2–3 s post-reveal stalls on first launch and the seam-class recompiles the July plan measured | M | load-cell frame p99 / max; `browser.pipelines.sync` empty after reveal; light snapshot constant |
...
| 2.7b | **Lava lake remake** — design in [ODYSSEY_EARTH_CORE_LAVA_LAKE_REMAKE_2026-08.md](ODYSSEY_EARTH_CORE_LAVA_LAKE_REMAKE_2026-08.md): bake the calibrated primitive into a periodic 3D texture through the existing `fbm(…, sn)` seam (Stage 1, look byte-identical by construction), then three small doc-requested look deltas (Stage 2). | lake pipeline 2.0 s → ≤ 0.4 s cold; Lane B fragment cost est. 3–6× lower | M (4 sessions + 3) | gates in that doc §4 |
