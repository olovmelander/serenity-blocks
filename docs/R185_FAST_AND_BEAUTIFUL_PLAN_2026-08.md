...
| 2.16 | **Next: the world bake is the pole again, and chapter 1 owns the barrier.** With the CPU steps, the icon atlas, the second passes and One World out of the way, a cold start is ~2.85 s of which `compiles` is 1,417 (chapter 1 alone) and `world` 150 (the build, plus whatever bake wait remains — it reappears the moment anything else gets faster: a run with a 1,437 ms barrier showed `world` 658). Levers: (a) chapter 1's own pipeline count — the 2.13 census pattern applied to variants (how many distinct materials does the reveal frame really need?); (b) start the bake before the Odyssey chunk loads (it needs only quality; the menu could kick it), or split the sun march, its longest remaining lane. | −0.3…−0.6 s cold | M | `compiles`, `world` buckets |
