...
| 2.15 | **Next: the compile barrier is now the startup.** With the CPU steps and the bakes off the critical path, `compiles` is ~1.75 s of a 3.2 s cold start and the pole is chapter 1 (compile-breakdown ch1 ≈ 2.5 s wall, overlapping one-world 1.9, board 1.4, corridor 1.0, breach 0.6 in a 6-wide pool). Two levers, both now unlocked: (a) item 2.11's live-loop compile path means corridor / breach / board presentation no longer have to sit inside the pre-reveal barrier — they can warm right after the reveal, before the first transition, leaving ch1 the GPU to itself; (b) fewer/cheaper ch1 pipelines (the 2.13 census pattern applied to variants, not just sides). | −0.4…−0.9 s cold | M | `compiles` bucket; first-transition hitch must stay 0 |
