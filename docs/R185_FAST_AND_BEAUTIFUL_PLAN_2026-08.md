...
| 2.16 | **The startup is BAKE-BOUND — the finding that reframes what is left (2026-08-22).** Three measured experiments all said the same thing. (a) Fan-out width 20 vs 6: `compiles` 1,417 → 858 but total 2,847 → **3,338** — r185 builds every render object's node graph on the MAIN thread between yields, so widening the pool relabels that cost into `nodes`/`world`/`creates` instead of removing it. (b) Deferring corridor+breach past the reveal: total → 3,420, because the pre-reveal warm-up replay renders the whole scene and compiles them serially instead (item 2.15). (c) The GPU device warm: `renderer` → 42 ms, total → 3,267, because that await was free bake time. **So**: from `renderer` through `world` the critical path is the world bake (~0.9–1.4 s wall in dev; its own pole is the relief-bands → sun-march chain, and the band count is already at its knee — 1,388 / 1,412 / 1,514 ms at 3 / 6 / 8 bands, each band being another Worker to spawn), and `compiles` (1,417) is mostly main-thread node building for chapter 1. Levers, in order: **start the bake at menu time** (it needs only quality for the terrain+plates lanes; gate on the saved mode so a non-Odyssey player pays nothing, and measure through a flag), then **fewer/cheaper node graphs for chapter 1**, then switch 2.5 on. | −0.4…−0.8 s cold | M | `world`, `nodes`, `compiles` buckets |
