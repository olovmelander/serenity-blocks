...
| 4.1 | **CPU-profile the idle frame first** (chrome-devtools trace at the pinned ch1 station, 10 s): self-time of `_renderObjectDirect`, `Bindings._update`, `writeBuffer`, `updateMatrixWorld`, app update. Nothing below is admissible until this exists. **MEASURED 2026-08-24, corrected 2026-08-25 — the answer is CPU, but by less than first reported.** Worst idle GPU p95 across 61 themes is **2.42 ms** (summer) against the 9 ms `gpuMaxMs` split, and no theme exceeds 16.67 ms wall. `cpuMaxMs` is 6 and **neon-district measures ~10.4 ms idle CPU-submit p95 at 1,856 draw calls (n = 4: 10.5/10.7/9.9/10.5, about 1.7x over)**; golden-forest straddles the budget at 4.7-7.9 ms (n = 4) and ice-temple sits at 6.6 ms (n = 1). The first published figures — 23.3 and 14.3, called a 3.9x breach — were an instrument defect (double-counted nested render entries, fixed in `71fcf9a9`); 20.4 ms of CPU inside a 7.8 ms frame is impossible and that check was not applied before publishing. Exactly 2 of 61 themes were affected. Idle wall p95 is bimodal at ~8.2 / ~15.9 ms — a 120 Hz panel versus themes honouring the 60 Hz target, i.e. pacing, not cost. Phase 4 is a submission-cost phase, not a fill phase. | A | prerequisite | S | trace attached to the report |
