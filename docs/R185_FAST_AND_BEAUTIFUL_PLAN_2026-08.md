...
| 4.1 | **CPU-profile the idle frame first** (chrome-devtools trace at the pinned ch1 station, 10 s): self-time of `_renderObjectDirect`, `Bindings._update`, `writeBuffer`, `updateMatrixWorld`, app update. Nothing below is admissible until this exists. **MEASURED 2026-08-24 — the answer is CPU, decisively.** Worst idle GPU p95 across 61 themes is **2.42 ms** (summer) against the 9 ms `gpuMaxMs` split, and no theme exceeds 16.67 ms wall. But `cpuMaxMs` is 6 and **neon-district measures 23.3 ms idle CPU-submit p95 at 1,856 draw calls (3.9x over)**, golden-forest 14.3 ms at 1,101 draws, ice-temple 6.6 ms. Idle wall p95 is bimodal at ~8.2 / ~15.9 ms — a 120 Hz panel versus themes honouring the 60 Hz target, i.e. pacing, not cost. Phase 4 is a submission-cost phase, not a fill phase. | A | prerequisite | S | trace attached to the report |
