...
| 2.16b | **Next: `compiles` — 1,417 ms of a 2,847 ms cold start, and the only bucket left with real mass.** Inside it chapter 1 is ~2,000 ms of wall in a 6-wide pool, and item 2.16(a) established the cost is main-thread NODE BUILDING, not GPU compile time — so the lever is fewer and cheaper node graphs, not more parallelism (width 20 already proved that direction is negative). Apply the item 2.13 census pattern to variants rather than sides: enumerate chapter 1's materials by emitted pipeline, find the ones whose graphs are duplicated per-variant or per-instance, and collapse them. Rows 2.8 and 2.10 name the specific shaders (lava-fall 0.34 s, three Earth Core Standard materials ~0.47 s each, `MeshBasicNodeMaterial_18` 0.41 s). | −0.4…−0.9 s cold | M | `compiles` bucket; compile-breakdown ch1; first-transition hitch stays 0 |
