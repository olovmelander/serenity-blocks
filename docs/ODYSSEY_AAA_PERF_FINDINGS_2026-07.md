...
**Status: SHIPPED — DEFAULT-ON.** The per-launch gen cost (originally +221 ms) was the ship
blocker (it would regress warm-cache launches, which get no compile benefit). Root cause: the
sampler only reads `.r` but the generator built 3 RGB channels — dropping to a single RedFormat
Float32 channel cut the gen ~3× to **within measurement noise** (the earth-core `creates` bucket
returned to the procedural ~937 ms level). With the gen no longer a meaningful warm-cache cost,
`EARTH_CORE_BAKE_NOISE` was flipped to **default-on**: measured no-flag baked cold compile
~1145 ms (vs ~2114 procedural), and the baked earth-core renders correctly in-scene incl. the
rock-cluster/obsidian-geode `moltenRockField` consumers up close (sharp analytic veins, natural
baked crust). Escape hatch: `?earthCoreBakeNoise=0` (dev) / `localStorage.serenity.earthCoreBakeNoise='0'`
(packaged Electron) forces analytic.
