...
| [THEME_FLEET_SWEEP_2026-08.md](THEME_FLEET_SWEEP_2026-08.md) | Evidence | **ACTIVE — fills the plan's Phase 3/4/5 rows with per-theme evidence; not a parallel roadmap.** All 61 themes: static census (Part A), ONE measured GPU sweep on a single adapter — 59/61 admissible, re-measured after two instrument corrections (Part B, §20; the correction post-mortem is §21) — and the Stage-4 fix campaign (Part C): seven themes landed on the bound-compile recipe (−46.5 % aggregate first-frame across the six kept), two reverted and recorded. Also the governance record for the theme perf lane: cells at reports/theme-perf/ (one runId = one comparable snapshot), admissibility rules, and which cell fields tolerate an n=1 baseline (§18). |
...
| [ODYSSEY_WORLD_BAKE_WORKER_2026-08.md](ODYSSEY_WORLD_BAKE_WORKER_2026-08.md) | Design | **LANDED 2026-08-21** (plan item 2.1). The One World boot bakes (five plates + the cloud-field sculpt) run in a module Worker started before `renderer.init()`; nodes seat from the relief stage, the world builds after the nodes from landed arrays. Stage inventory with measured worker ms, the pure/wrap split, loader contract (synchronous twin on first access), why no placeholder phase, rollback `?odysseyWorldBakeSync=1`. Cold startup 5,580 → 4,008 ms. |
