---
trigger: always_on
description: WebGPU/TSL validation loop for themes and Odyssey chapters
---
...
2. **Screenshot to verify**: open the page (Antigravity's built-in browser, or the
   `chrome-devtools` MCP), wait for `window.__PLAYGROUND_READY__ === true`, capture the
   canvas, and read the console for WebGPU validation errors. Use `?t=<seconds>` for
   reproducible, phase-locked shots.
...
The `webgpu-threejs-tsl` skill (in `.agents/skills/`, auto-activates on WebGPU/TSL work)
