...
const BASELINE_PARAMS = {
    swedishForestBaseline: '1',
    swedishForestSeed: '1234',
    swedishForestFixedDt: '16.666',
};
const SCENARIOS = [
    { name: 'default', params: {} },
    { name: 'force_webgl', params: { forceWebGL: '1' } },
    { name: 'no_post', params: { swedishForestNoPost: '1' } },
    { name: 'no_mrt', params: { swedishForestNoMRT: '1' } },
    { name: 'no_compute', params: { swedishForestNoCompute: '1' } },
];
...
async function bootstrapSwedishForestScene(win) {
    const script = `
        (async () => {
            ...
            try {
                await app.themeManager.switchTheme('swedish-forest', true);
            } catch (error) {
                return { ok: false, reason: 'Failed to switch to swedish-forest: ' + (error?.message || String(error)) };
            }

            const themeReady = await waitFor(
                () => {
                    const theme = window.themeManager?.activeTheme;
                    return window.themeManager?.activeThemeName === 'swedish-forest'
                        && !!theme?.renderer
                        && !!theme?.scene
                        && !!theme?.camera;
                },
            ...
            if (!themeReady) {
                return { ok: false, reason: 'Swedish Forest theme was not ready in time.' };
            }
...
async function runSmokeScenario(scenario) {
    ...
    try {
        ...
        bootstrapResult = await bootstrapSwedishForestScene(win);
    ...
    const sceneReadyLogged = consoleEntries.some((entry) => entry.message.includes('[SwedishForest] Scene ready'));
    const backendLogged = consoleEntries.find((entry) => entry.message.includes('[SwedishForest] Backend:'))?.message || null;
    const hasValidThemeState = state?.activeThemeName === 'swedish-forest'
...
async function runSoak() {
    ...
    try {
        await win.loadURL(url);
        const bootstrapResult = await bootstrapSwedishForestScene(win);
        if (!bootstrapResult?.ok) {
            throw new Error(bootstrapResult?.reason || 'Unknown bootstrap failure');
        }
...
function buildSummaryMarkdown(smokeResults, soakResult, jsonPath) {
    ...
    return [
        '# Swedish Forest Phase 1 Validation Report',
        '',
